<?php
global $base_url;
?>
<div class="bg-img1 size1 overlay1 p-t-24">
    <div class="flex-w flex-sa respon1">
        <div class="p-t-34 p-b-60 respon3">
            <p class="l1-txt1 p-b-10 respon2">
                Get more jobs done
            </p>

            <h3 class="l1-txt2 p-b-45 respon2 respon4">
                To earn more money
            </h3>

            <div class="cd100"></div>

        </div>

        <div class="bg0 wsize1 bor1 p-l-45 p-r-45 p-t-50 p-b-18 p-lr-15-sm">
            <h3 class="l1-txt3 txt-center p-b-43">
                Login
            </h3>
            <?php
            print $user_form;
            ?>
            <p class="s1-txt3 txt-center p-l-15 p-r-15 p-t-25">
                <a href="<?php echo $base_url; ?>/user/password" class="forgotpass">Forgot password?</a>
                <a href="<?php echo $base_url; ?>/user/register" class="create-an-account">Create an account</a>
            </p>
        </div>
    </div>
</div>